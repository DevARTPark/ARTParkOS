...
import { 
  ...
  ChevronLeft, Save, DollarSign, MessageSquare, Info,
  Plus, Trash2, TrendingUp, BarChart3, Check, X, ArrowRight,
  Briefcase, Rocket, CreditCard, Lock, Unlock, AlertCircle, ArrowDownCircle, RefreshCw, PieChart, Building2
} from 'lucide-react';
import { 
  XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, AreaChart, Area, BarChart, Bar, Legend, Cell
} from 'recharts';
...
// --- CONSTANTS ---
const RE_CATEGORIES = [
  "Others", "Salaries", "Subscriptions", "Cloud Costs", 
  "Office Expenses", "Maintenance", "Marketing"
];

const NRE_CATEGORIES = [
  "Others", "Hardware Purchase", "Prototyping", "R&D Setup", 
  "Software Build", "Legal & IP", "Office Setup"
];

const PROJECT_COLORS = ['#4f46e5', '#f97316', '#0ea5e9', '#8b5cf6', '#10b981', '#f43f5e'];
...
interface ReportDetail extends Omit<BaseReportDetail, 'projectUpdates'> {
    projectUpdates: (BaseReportDetail['projectUpdates'][0] & { expenses: Expense[] })[];
...
const adaptReports = (reports: any[]): ReportDetail[] => {
    ...
    return reports.map(r => ({
        ...r,
        projectUpdates: r.projectUpdates ? r.projectUpdates.map((p: any) => ({
            ...p,
            expenses: p.expenses ? p.expenses.map((e: any) => ({ 
                ...e, 
                type: e.type || 'NRE',
        ...
        startupUpdates: r.startupUpdates ? {
            ...r.startupUpdates,
            expenses: r.startupUpdates.expenses ? r.startupUpdates.expenses.map((e: any) => ({
                ...e,
                type: e.type || 'NRE',
                category: e.category || 'Others' 
            })) : []
...
export function FounderReviews() {
  ...
  const [reports, setReports] = useState<ReportDetail[]>(() => {
    try {
        ...
        return saved ? adaptReports(JSON.parse(saved)) : adaptReports(initialMonthlyReports);
    } catch (e) {
  ...
  const [newPointInput, setNewPointInput] = useState<Record<string, string>>({}); // Key: projectId_field
  ...
  const [newStartupExpenseInput, setNewStartupExpenseInput] = useState<ExpenseInput>({item: '', amount: '', type: 'NRE', category: 'Others', periodicity: 'Monthly'});
  ...
  const handleBackToList = () => {
    ...
    setNewStartupExpenseInput({item: '', amount: '', type: 'NRE', category: 'Others', periodicity: 'Monthly'});
  ...
  const handleAddPoint = (projectId: string, field: 'highlights' | 'risks') => {
    ...
    const project = selectedReport.projectUpdates.find(p => p.projectId === projectId);
    if (!project) return;
  ...
  const handleAddStartupTask = () => {
      ...
      const updatedReport = {
          ...
          startupUpdates: {
              ...selectedReport.startupUpdates,
              scheduleTasks: [...selectedReport.startupUpdates.scheduleTasks, newTask]
          }
      };
      setSelectedReport(updatedReport);
      setNewStartupTaskInput({ title: '', date: '', description: '' });
  ...
  const handleRemoveStartupTask = (taskId: string) => {
      ...
      const updatedReport = {
          ...
          startupUpdates: {
              ...selectedReport.startupUpdates,
              scheduleTasks: selectedReport.startupUpdates.scheduleTasks.filter(t => t.id !== taskId)
          }
      };
      setSelectedReport(updatedReport);
  };
                  ...
                  if (shouldAdd) {
                      ...
                      if (projectId) {
                          return {
                              ...report,
                              projectUpdates: report.projectUpdates.map(p => 
                                  p.projectId === projectId ? { ...p, expenses: [...(p.expenses || []), propagatedExpense] } : p
                              )
                          };
                      } else {
                          return {
  ...
  const handleAddProjectExpense = (projectId: string) => {
      ...
      setNewExpenseInput({ 
          ...newExpenseInput, 
          [projectId]: { item: '', amount: '', type: 'NRE', category: 'Others', periodicity: 'Monthly' } 
      });
  };
  ...
  const handleRemoveExpense = (projectId: string | null, expenseId: string) => {
          ...
          if (r.reportId === selectedReport.reportId) {
              if (projectId) {
                  ...
                  return {
                      ...
                      startupUpdates: {
                          ...r.startupUpdates,
                          expenses: r.startupUpdates.expenses.filter(e => e.id !== expenseId)
                      }
                  };
              }
          }
      ...
      setReports(updatedReports);
      const updatedSelected = updatedReports.find(r => r.reportId === selectedReport.reportId);
      if (updatedSelected) setSelectedReport(updatedSelected);
  };

  const handleExpenseInputChange = (projectId: string, field: keyof ExpenseInput, val: string) => {
  ...
  const handleStartupExpenseInputChange = (field: keyof ExpenseInput, val: string) => {
      if (field === 'type') {
          const newType = val as 'RE' | 'NRE';
    ...
    const data = reports.map(report => {
        ...
        report.projectUpdates.forEach(p => {
            ...
            (p.expenses || []).forEach((e: any) => {
                ...
                if (isNaN(val)) val = 0;
                if (e.type === 'RE') totalRE += val; else totalNRE += val;
                pTotal += val;
            });
  ...
  const getQuarterlyPromises = (reportMonthYear: string) => {
      ...
      else if (['July', 'August', 'September'].includes(month)) targetQuarter = 'Q2';
  ...
  const getStatusColor = (status: string) => {
    switch(status) {
      ...
      case 'Auto-Submitted': return 'bg-purple-100 text-purple-800 border-purple-200';
        ...
        <>
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                ...
                <div className="text-sm text-gray-500 flex items-center bg-white px-3 py-1 rounded-full border border-gray-200 shadow-sm">
                    ...
                    Today: <span className="font-semibold text-gray-900 ml-1">{new Date().toLocaleDateString('default', { month: 'short', day: 'numeric' })}</span>
                </div>
            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-2">
                    <div className="lg:col-span-2 space-y-4">
                        {reports.map((report) => {
                            ...
                            return (
                                <Card key={report.reportId} className={`transition-all ${locked ? 'bg-gray-50' : future ? 'opacity-60' : 'hover:shadow-md border-l-4 border-l-blue-400'}`}>
                                    <CardContent className="p-5 flex flex-col md:flex-row items-center justify-between gap-4">
                                        <div className="flex items-center space-x-4">
                                            <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                                                displayStatus === 'Reviewed' ? 'bg-green-100 text-green-600' :
                                                displayStatus === 'Submitted' ? 'bg-blue-100 text-blue-600' : 
                                                locked ? 'bg-purple-100 text-purple-600' : 
                                                future ? 'bg-gray-100 text-gray-300' : 'bg-yellow-100 text-yellow-600'
                                            }`}>
                                                {locked ? <Lock className="w-5 h-5" /> : future ? <Calendar className="w-5 h-5"/> : <Unlock className="w-5 h-5" />}
                                            </div>
                                            <div>
                                        ...
                                        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                                            <Badge variant={
                                                displayStatus === 'Reviewed' || displayStatus === 'Submitted' ? 'success' :
                                                displayStatus === 'Auto-Submitted' ? 'neutral' : 
                                                future ? 'neutral' : 'warning'
                                            }>
                                                {displayStatus}
                                            </Badge>
                                            ...
                                            <Button 
                                                ...
                                                disabled={future}
                                            >
                                                {locked ? 'View Report' : (displayStatus === 'Submitted' ? 'Edit / View' : 'Start Report')}
                                            </Button>
                                        </div>
                                    </CardContent>
                                </Card>
                    ...
                    <div>
                        <Card className="bg-blue-50 border-blue-100 h-full">
                            <CardHeader>
                                <CardTitle className="flex items-center text-blue-800"><Target className="w-5 h-5 mr-2" /> Current Focus</CardTitle>
                                <p className="text-xs text-blue-600 mt-1">Checkpoints from Quarterly Strategy</p>
                            </CardHeader>
                ...
                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                    ...
                    <Card className="border-t-4 border-t-emerald-500">
                        <CardHeader className="flex flex-row items-center justify-between">
                            <div>
                                <CardTitle className="flex items-center text-emerald-900"><DollarSign className="w-5 h-5 mr-2" /> Monthly Budget Sheet</CardTitle>
                                <p className="text-xs text-emerald-600 mt-1">Consolidated view of RE & NRE expenses across Startup Level & Projects.</p>
                            </div>
                            <Button variant="outline" size="sm" onClick={() => window.print()} leftIcon={<Upload className="w-4 h-4 rotate-180" />}>Export Sheet</Button>
                        </CardHeader>
                        <CardContent>
                                ...
                                <table className="w-full text-sm">
                                    <thead className="bg-gray-50 text-gray-500 font-bold uppercase text-xs">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3 text-right text-indigo-600">Recurring (RE)</th>
                                            <th className="px-4 py-3 text-right text-orange-600">Non-Recurring (NRE)</th>
                                            <th className="px-4 py-3 text-right text-gray-900">Total Spent</th>
                                    ...
                                    <tbody className="divide-y divide-gray-100">
                                        {budgetSheetData.map((row, idx) => (
                                            <tr key={idx} className="hover:bg-gray-50/50">
                                                <td className="px-4 py-4 font-bold text-gray-800">{row.month}</td>
                                                <td className="px-4 py-4">
                                                    <div className="flex flex-col gap-1">
                                                        {row.breakdown.map((b, i) => (
                                                            <div key={i} className="flex justify-between text-xs text-gray-500 w-full max-w-[200px]">
                                                                <span className={b.name === 'Startup Level' ? 'font-bold text-indigo-900' : ''}>{b.name}:</span>
                                                                <span>₹{b.total}</span>
                                                            </div>
                                                        ))}
                                                    </div>
                                                </td>
                                                <td className="px-4 py-4 text-right font-mono text-indigo-600">₹{row.totalRE}</td>
                                                <td className="px-4 py-4 text-right font-mono text-orange-600">₹{row.totalNRE}</td>
                                                <td className="px-4 py-4 text-right font-mono font-bold text-gray-900">₹{row.total}</td>
                                                <td className="px-4 py-4 text-center">
                                                    <Badge variant={row.status === 'Submitted' ? 'success' : row.status === 'Pending' ? 'warning' : 'neutral'}>
                                                        {row.status}
                                                    </Badge>
                                                </td>
                                            </tr>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         ...
                         <Card>
                            <CardHeader><CardTitle className="text-sm">Expense Trend (RE vs NRE)</CardTitle></CardHeader>
                            <CardContent className="h-64">
                                <ResponsiveContainer width="100%" height="100%">
                                    ...
                                    <BarChart data={[...budgetSheetData].reverse()}>
                                        ...
                                        <Tooltip />
                                        <Legend />
                                        <Bar dataKey="totalRE" name="Recurring" stackId="a" fill="#4f46e5" />
                                        <Bar dataKey="totalNRE" name="Non-Recurring" stackId="a" fill="#f97316" />
                                    </BarChart>
                                </ResponsiveContainer>
                            </CardContent>
                         </Card>
                         ...
                         <Card>
                            <CardHeader><CardTitle className="text-sm">Project-wise Spending (Total)</CardTitle></CardHeader>
                            <CardContent className="h-64">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={projectWiseExpenseData} layout="vertical" margin={{top: 5, right: 30, left: 40, bottom: 5}}>
                                        ...
                                        <Bar dataKey="value" name="Total Spend" barSize={20} radius={[0, 4, 4, 0]}>
                    ...
                    <Card>
                        <CardHeader><CardTitle className="text-sm">Budget Utilization</CardTitle></CardHeader>
                         <CardContent className="flex items-center justify-center h-48 text-gray-400 text-sm italic">
                             <PieChart className="w-8 h-8 mr-2"/> Detailed budget vs actuals analytics coming soon...
                         </CardContent>
                    </Card>
                </div>
            )}
             ...
             <div className="flex items-center justify-between mb-6 sticky top-0 bg-gray-50 py-4 z-10 border-b border-gray-200">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-500">
                            <Badge variant={isLockedForEditing ? 'neutral' : 'success'}>{getReportDisplayStatus(selectedReport)}</Badge>
                            <span>•</span>
                            <span className={isLockedForEditing ? 'text-red-500 font-medium' : 'text-blue-600'}>
                                {isLockedForEditing 
                                    ? 'Submissions Closed' 
                                    : `Editable until ${getDeadlineForMonth(selectedReport.month).toLocaleDateString()} 11:59 PM`
                                }
                            </span>
                        </div>
                    </div>
                ...
                {!isLockedForEditing && (
                    <div className="flex items-center gap-3">
                        ...
                        <Button onClick={handleSubmitReport} leftIcon={<Upload className="w-4 h-4" />}>
                            {selectedReport.status === 'Submitted' ? 'Update Submission' : 'Submit Report'}
                        </Button>
                    </div>
                )}
            </div>
                ...
                <Card className="mb-8 border-l-4 border-l-purple-500 bg-purple-50 shadow-sm">
                    <CardHeader>
                        <CardTitle className="flex items-center text-purple-900">
                            <MessageSquare className="w-5 h-5 mr-2" /> Reviewer Feedback & Gaps
                        </CardTitle>
                    </CardHeader>
                    <CardContent>
                        {selectedReport.artparkRemarks ? (
                            <div className="prose prose-sm text-purple-900">
                                <p className="italic mb-4 text-lg">"{selectedReport.artparkRemarks}"</p>
                                <div className="p-4 bg-white rounded-lg border border-purple-100 shadow-sm">
                                    <h5 className="font-bold text-xs uppercase text-purple-700 mb-2 flex items-center">
                                        <AlertCircle className="w-3 h-3 mr-1"/> Action Items / Gaps Identified
                                    </h5>
                                    <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                                        <li>Update financial projections for next month.</li>
                                        <li>Clarify risk mitigation strategy for supply chain delays.</li>
                                    </ul>
                                </div>
                            </div>
                        ) : (
                            <p className="text-sm text-gray-500 italic flex items-center"><Info className="w-4 h-4 mr-2"/> No reviewer feedback available yet.</p>
                        )}
                    </CardContent>
                </Card>
            )}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 pb-12">
                <div className="xl:col-span-2 space-y-8">
                    ...
                    <Card className="border-l-4 border-l-indigo-600 shadow-sm">
                        <CardHeader className="bg-indigo-50 border-b border-indigo-100 pb-3">
                            <div className="flex justify-between items-center">
                                <div className="flex items-center gap-3">
                                    ...
                                    <CardTitle className="text-lg text-indigo-900">Startup Level Updates</CardTitle>
                                </div>
                                <Badge variant="neutral" className="bg-white">General</Badge>
                            </div>
                        </CardHeader>
                        <CardContent className="p-6 space-y-6">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="bg-white rounded-md border border-gray-200 p-2 space-y-1 min-h-[80px]">
                                        {getPoints(selectedReport.startupUpdates.highlights).length > 0 ? (
                                            getPoints(selectedReport.startupUpdates.highlights).map((point, i) => (
                                                <div key={i} className="flex items-start group text-sm p-1 hover:bg-gray-50 rounded">
                                                    <span className="text-green-500 mr-2 mt-1">•</span>
                                                    <span className="flex-1 text-gray-800">{point}</span>
                                                    {!isLockedForEditing && <button onClick={() => handleRemoveStartupPoint('highlights', i)} className="text-gray-300 hover:text-red-500"><X className="w-3 h-3"/></button>}
                                                </div>
                                            ))
                                        ) : <p className="text-xs text-gray-400 italic p-2">No highlights.</p>}
                                        {!isLockedForEditing && (
                                            <div className="flex gap-2 mt-2 pt-2 border-t border-gray-100">
                                                <Input className="h-7 text-xs" placeholder="Add highlight..." value={newStartupPointInput.highlights} onChange={e => setNewStartupPointInput({...newStartupPointInput, highlights: e.target.value})} onKeyDown={e => e.key === 'Enter' && handleAddStartupPoint('highlights')} />
                                                <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => handleAddStartupPoint('highlights')}><Plus className="w-4 h-4"/></Button>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    <label className="text-xs font-bold text-gray-500 uppercase flex items-center gap-2"><ArrowDownCircle className="w-3 h-3 text-red-600" /> Lowlights</label>
                                    <div className="bg-white rounded-md border border-gray-200 p-2 space-y-1 min-h-[80px]">
                                        {getPoints(selectedReport.startupUpdates.risks).length > 0 ? (
                                            getPoints(selectedReport.startupUpdates.risks).map((point, i) => (
                                                <div key={i} className="flex items-start group text-sm p-1 hover:bg-gray-50 rounded">
                                                    <span className="text-red-500 mr-2 mt-1">•</span>
                                                    <span className="flex-1 text-gray-800">{point}</span>
                                                    {!isLockedForEditing && <button onClick={() => handleRemoveStartupPoint('risks', i)} className="text-gray-300 hover:text-red-500"><X className="w-3 h-3"/></button>}
                                                </div>
                                            ))
                                        ) : <p className="text-xs text-gray-400 italic p-2">No lowlights.</p>}
                                        {!isLockedForEditing && (
                                            <div className="flex gap-2 mt-2 pt-2 border-t border-gray-100">
                                                <Input className="h-7 text-xs" placeholder="Add lowlight..." value={newStartupPointInput.risks} onChange={e => setNewStartupPointInput({...newStartupPointInput, risks: e.target.value})} onKeyDown={e => e.key === 'Enter' && handleAddStartupPoint('risks')} />
                                                <Button size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => handleAddStartupPoint('risks')}><Plus className="w-4 h-4"/></Button>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-100">
                                {/* STARTUP MILESTONES */}
                                <div>
                                    <label className="text-xs font-bold text-gray-500 uppercase mb-3 block">Startup Milestones</label>
                                    <div className="space-y-2 mb-4">
                                        {selectedReport.startupUpdates.scheduleTasks.length > 0 ? selectedReport.startupUpdates.scheduleTasks.map(task => (
                                            <div key={task.id} className="bg-white border border-gray-200 p-3 rounded-lg">
                                                ...
                                                <div className="flex justify-between items-end">
                                                    ...
                                                    {!isLockedForEditing && <button onClick={() => handleRemoveStartupTask(task.id)} className="text-gray-400 hover:text-red-500"><Trash2 className="w-3 h-3" /></button>}
                                                </div>
                                            </div>
                                    ...
                                    {!isLockedForEditing && (
                                        <div className="flex flex-col gap-2 bg-gray-50 p-3 rounded-lg border border-gray-200">
                                            <Input placeholder="Milestone Title" className="h-7 text-xs" value={newStartupTaskInput.title} onChange={(e) => setNewStartupTaskInput({...newStartupTaskInput, title: e.target.value})} />
                                            <Input type="date" className="h-7 text-xs" value={newStartupTaskInput.date} onChange={(e) => setNewStartupTaskInput({...newStartupTaskInput, date: e.target.value})} />
                                            <Input placeholder="Description" className="h-7 text-xs" value={newStartupTaskInput.description} onChange={(e) => setNewStartupTaskInput({...newStartupTaskInput, description: e.target.value})} />
                                            <Button size="sm" variant="secondary" className="w-full h-7 text-xs mt-1" onClick={handleAddStartupTask} disabled={!newStartupTaskInput.title}>Add</Button>
                                        </div>
                                    )}
                                </div>
                                ...
                                <div className="border-l border-gray-100 pl-6">
                                    <label className="text-xs font-bold text-gray-500 uppercase mb-3 block flex items-center justify-between">
                                        <div className="flex items-center"><CreditCard className="w-3 h-3 mr-1.5"/> Startup Expense Sheet</div>
                                        <Badge variant="neutral">Total: ₹{getExpenseTotal(selectedReport.startupUpdates.expenses)}</Badge>
                                    </label>
                                    <div className="border border-gray-200 rounded-lg overflow-hidden mb-4">
                                        <table className="w-full text-xs">
                                            ...
                                            <tbody className="divide-y divide-gray-100">
                                                {(selectedReport.startupUpdates.expenses || []).length > 0 ? (selectedReport.startupUpdates.expenses).map((exp) => (
                                                    <tr key={exp.id} className="bg-white">
                                                        <td className="px-3 py-2">
                                                            <div className="text-gray-900 font-medium">{exp.item}</div>
                                                            <div className="text-[10px] text-gray-400 flex items-center gap-2">
                                                                <span>{exp.category}</span>
                                                                {exp.type === 'RE' && exp.periodicity && <span className="bg-indigo-50 text-indigo-700 px-1 rounded flex items-center"><RefreshCw className="w-2 h-2 mr-1"/> {exp.periodicity}</span>}
                                                            </div>
                                                        </td>
                                                        <td className="px-2 py-2 text-center">
                                                            <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${exp.type === 'RE' ? 'bg-indigo-50 text-indigo-700' : 'bg-orange-50 text-orange-700'}`}>{exp.type}</span>
                                                        </td>
                                                        <td className="px-3 py-2 text-right font-mono">₹{exp.amount}</td>
                                                        {!isLockedForEditing && <td className="px-2 py-2 text-center"><button onClick={() => handleRemoveExpense(null, exp.id)} className="text-gray-300 hover:text-red-500"><X className="w-3 h-3"/></button></td>}
                                                    </tr>
                                                )) : <tr><td colSpan={4} className="px-3 py-4 text-center text-gray-400 italic">No expenses recorded.</td></tr>}
                                            </tbody>
                                        </table>
                                    </div>
                                    {!isLockedForEditing && (
                                        <div className="flex flex-col gap-2 bg-gray-50 p-3 rounded-lg border border-gray-200">
                                            <div className="flex gap-2">
                                                <Input placeholder="Item Name" className="h-7 text-xs flex-1" value={newStartupExpenseInput.item} onChange={(e) => handleStartupExpenseInputChange('item', e.target.value)} />
                                                <Input type="number" placeholder="₹ Amount" className="h-7 text-xs w-24" value={newStartupExpenseInput.amount} onChange={(e) => handleStartupExpenseInputChange('amount', e.target.value)} />
                                            </div>
                                            <div className="flex gap-2 items-center">
                                                <select className="h-7 text-xs border border-gray-300 rounded px-1 bg-white focus:outline-none w-20" value={newStartupExpenseInput.type} onChange={(e) => handleStartupExpenseInputChange('type', e.target.value)}>
                                                    <option value="NRE">NRE</option>
                                                    <option value="RE">RE</option>
                                                </select>
                                                
                                                <select className="h-7 text-xs border border-gray-300 rounded px-1 bg-white focus:outline-none flex-1" value={newStartupExpenseInput.category} onChange={(e) => handleStartupExpenseInputChange('category', e.target.value)}>
                                                    {(newStartupExpenseInput.type === 'RE' ? RE_CATEGORIES : NRE_CATEGORIES).map(cat => (
                                                        <option key={cat} value={cat}>{cat}</option>
                                                    ))}
                                                </select>

                                                {newStartupExpenseInput.type === 'RE' && (
                                                    <select className="h-7 text-xs border border-gray-300 rounded px-1 bg-white focus:outline-none w-24" value={newStartupExpenseInput.periodicity} onChange={(e) => handleStartupExpenseInputChange('periodicity', e.target.value)}>
                                                        <option value="Monthly">Monthly</option>
                                                        <option value="Quarterly">Quarterly</option>
                                                        <option value="Yearly">Yearly</option>
                                                    </select>
                                                )}
                                                <Button size="sm" variant="secondary" className="h-7 text-xs px-3" onClick={handleAddStartupExpense}><Plus className="w-3 h-3"/></Button>
                                            </div>
                                        </div>
                                    )}
                                </div>
                    ...
                    {selectedReport.projectUpdates.map((project, index) => (
                        <Card key={project.projectId} className="border-l-4 border-l-blue-500 shadow-sm">
                            ...
                            <CardContent className="p-6 space-y-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-gray-500 uppercase flex items-center gap-2">
                                            <CheckCircle2 className="w-3 h-3 text-green-600" /> Highlights
                                        </label>
                                            ...
                                            {getPoints(project.highlights).length > 0 ? (
                                                getPoints(project.highlights).map((point, i) => (
                                                    <div key={i} className="flex items-start group text-sm p-1 hover:bg-gray-50 rounded">
                                                        ...
                                                        <span className="flex-1 text-gray-800">{point}</span>
                                                        {!isLockedForEditing && (
                                                            <button onClick={() => handleRemovePoint(project.projectId, 'highlights', i)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                <X className="w-3 h-3" />
                                                            </button>
                                                        )}
                                                    </div>
                                                ))
                                            ) : <p className="text-xs text-gray-400 italic p-2">No highlights added yet.</p>}
                                            ...
                                            {!isLockedForEditing && (
                                                <div className="flex items-center gap-2 pt-2 border-t border-gray-100 mt-2">
                                                    <Input 
                                                        ...
                                                        value={newPointInput[`${project.projectId}_highlights`] || ''}
                                                        onChange={(e) => setNewPointInput({...newPointInput, [`${project.projectId}_highlights`]: e.target.value})}
                                                        onKeyDown={(e) => {
                                                            if (e.key === 'Enter') handleAddPoint(project.projectId, 'highlights');
                                                        }}
                                                    />
                                                    <Button 
                                                        size="sm" 
                                                        ...
                                                        onClick={() => handleAddPoint(project.projectId, 'highlights')}
                                                    >
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-bold text-gray-500 uppercase flex items-center gap-2">
                                            <ArrowDownCircle className="w-3 h-3 text-red-600" /> Lowlights
                                        </label>
                                            ...
                                            {getPoints(project.risks).length > 0 ? (
                                                getPoints(project.risks).map((point, i) => (
                                                    <div key={i} className="flex items-start group text-sm p-1 hover:bg-gray-50 rounded">
                                                        ...
                                                        <span className="flex-1 text-gray-800">{point}</span>
                                                        {!isLockedForEditing && (
                                                            <button onClick={() => handleRemovePoint(project.projectId, 'risks', i)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity">
                                                                <X className="w-3 h-3" />
                                                            </button>
                                                        )}
                                                    </div>
                                                ))
                                            ) : <p className="text-xs text-gray-400 italic p-2">No lowlights added yet.</p>}
                                            ...
                                            {!isLockedForEditing && (
                                                <div className="flex items-center gap-2 pt-2 border-t border-gray-100 mt-2">
                                                    <Input 
                                                        ...
                                                        value={newPointInput[`${project.projectId}_risks`] || ''}
                                                        onChange={(e) => setNewPointInput({...newPointInput, [`${project.projectId}_risks`]: e.target.value})}
                                                        onKeyDown={(e) => {
                                                            if (e.key === 'Enter') handleAddPoint(project.projectId, 'risks');
                                                        }}
                                                    />
                                                    <Button 
                                                        size="sm" 
                                                        variant="ghost" 
                                                        className="h-8 w-8 p-0 text-blue-600 hover:bg-blue-50"
                                                        onClick={() => handleAddPoint(project.projectId, 'risks')}
                                                    >
                                                        <Plus className="w-4 h-4" />
                                                    </Button>
                                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-100">
                                        ...
                                        <div className="space-y-2 mb-4">
                                            {project.scheduleTasks.length > 0 ? project.scheduleTasks.map(task => (
                                                <div key={task.id} className="bg-white border border-gray-200 p-3 rounded-lg">
                                                    ...
                                                    <div className="flex justify-between items-end">
                                                        <p className="text-xs text-gray-500 italic flex-1 mr-2">{task.description}</p>
                                                        {!isLockedForEditing && <button onClick={() => handleRemoveTask(project.projectId, task.id)} className="text-gray-400 hover:text-red-500"><Trash2 className="w-3 h-3" /></button>}
                                                    </div>
                                                </div>
                                            )) : <p className="text-xs text-gray-400 italic">No milestones set.</p>}
                                        </div>
                                        ...
                                        {!isLockedForEditing && (
                                            <div className="flex flex-col gap-2 bg-gray-50 p-3 rounded-lg border border-gray-200">
                                                ...
                                                >
                                                    ...
                                                    <optgroup label={`AIRL ${project.currentAIRL + 1} Assessment Criteria`}>
                                    ...
                                    <div className="border-l border-gray-100 pl-6">
                                        <label className="text-xs font-bold text-gray-500 uppercase mb-3 block flex items-center justify-between">
                                            <div className="flex items-center"><CreditCard className="w-3 h-3 mr-1.5"/> Expense Sheet</div>
                                            <Badge variant="neutral">Total: ₹{getExpenseTotal(project.expenses || [])}</Badge>
                                        </label>
                                        ...
                                        <div className="border border-gray-200 rounded-lg overflow-hidden mb-4">
                                            <table className="w-full text-xs">
                                                ...
                                                <tbody className="divide-y divide-gray-100">
                                                    {(project.expenses || []).length > 0 ? (project.expenses as any[]).map((exp) => (
                                                        <tr key={exp.id} className="bg-white">
                                                            <td className="px-3 py-2">
                                                                <div className="text-gray-900 font-medium">{exp.item}</div>
                                                                <div className="text-[10px] text-gray-400 flex items-center gap-2">
                                                                    ...
                                                                    {exp.type === 'RE' && exp.periodicity && (
                                                                        <span className="bg-indigo-50 text-indigo-700 px-1 rounded flex items-center">
                                                                            <RefreshCw className="w-2 h-2 mr-1"/> {exp.periodicity}
                                                                        </span>
                                                                    )}
                                                                </div>
                                                            </td>
                                                            ...
                                                            {!isLockedForEditing && (
                                                                <td className="px-2 py-2 text-center">
                                                                    <button onClick={() => handleRemoveExpense(project.projectId, exp.id)} className="text-gray-300 hover:text-red-500"><X className="w-3 h-3"/></button>
                                                                </td>
                                                            )}
                                                        </tr>
                                                    )) : (
                                                        <tr><td colSpan={4} className="px-3 py-4 text-center text-gray-400 italic">No expenses recorded this month.</td></tr>
                                                    )}
                                                </tbody>
                                                <tfoot className="bg-gray-50 font-bold border-t border-gray-200">
                                                    <tr>
                                                        <td colSpan={2} className="px-3 py-2 text-gray-600">Total</td>
                                                        <td className="px-3 py-2 text-right text-gray-900">₹{getExpenseTotal(project.expenses || [])}</td>
                                            ...
                                            <div className="flex flex-col gap-2 bg-gray-50 p-3 rounded-lg border border-gray-200">
                                                ...
                                                <div className="flex gap-2 items-center">
                                                    <select 
                                                        ...
                                                        value={newExpenseInput[project.projectId]?.type || 'NRE'}
                                                        onChange={(e) => handleExpenseInputChange(project.projectId, 'type', e.target.value)}
                                                    >
                                                        <option value="NRE">NRE</option>
                                                        <option value="RE">RE</option>
                                                    </select>
                                                    ...
                                                    <select 
                                                        ...
                                                        value={newExpenseInput[project.projectId]?.category || 'Others'}
                                                        onChange={(e) => handleExpenseInputChange(project.projectId, 'category', e.target.value)}
                                                    >
                                                        {(newExpenseInput[project.projectId]?.type === 'RE' ? RE_CATEGORIES : NRE_CATEGORIES).map(cat => (
                                                            <option key={cat} value={cat}>{cat}</option>
                                                        ))}
                                                    </select>

                                                    {/* Periodicity Selector (Only if RE) */}
                                                    {newExpenseInput[project.projectId]?.type === 'RE' && (
                                                        <select 
                                                            className="h-7 text-xs border border-gray-300 rounded px-1 bg-white focus:outline-none w-24"
                                                            value={newExpenseInput[project.projectId]?.periodicity || 'Monthly'}
                                                            onChange={(e) => handleExpenseInputChange(project.projectId, 'periodicity', e.target.value)}
                                                    ...
                                                    <Button size="sm" variant="secondary" className="h-7 text-xs px-3" onClick={() => handleAddProjectExpense(project.projectId)}><Plus className="w-3 h-3"/></Button>
                                                </div>
                                            </div>
                                        )}
                                    </div>
