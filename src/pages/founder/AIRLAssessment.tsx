...
// Keep mockProjects for fallback, but remove airlQuestions import reliance
import { airlQuestions as defaultQuestions, projects as mockProjects } from '../../data/mockData';
import { 
...
export function AIRLAssessment() {
  ...
  if (!selectedProject) return <DashboardLayout role="founder" title="AIRL Assessment"><div>Loading...</div></DashboardLayout>;
  ...
  // Filter dynamic questions based on level logic
  ...
  useEffect(() => {
    ...
    if (savedData) {
      ...
      if (parsed.status === 'submitted' && parsed.taskId) {
        checkReviewerStatus(parsed.taskId);
      } else {
  ...
  const handleFinalSubmit = () => {
    ...
    const newTask = {
      ...
      description: isBaseline 
        ? `Initial validation for claimed Level ${targetLevel}.`
        : `Validation required for moving to Level ${targetLevel}.`,
      assigneeId: null, 
      status: "Pending",
      submissionKey: `artpark_assessment_${selectedProject.id}_AIRL${targetLevel}_${isBaseline ? 'BASELINE' : 'ROUTINE'}`
    };
  ...
  return (
    <DashboardLayout role="founder" title={isBaseline ? "Initial Baseline Assessment" : "AIRL Assessment"}>
      ...
      {/* Hidden Input moved outside loop */}
      ...
      {submissionStatus === 'submitted' ? (
        <div className="flex flex-col items-center justify-center h-[60vh] bg-white rounded-xl border border-gray-200 shadow-sm p-8 text-center animate-in fade-in zoom-in duration-500">
          ...
          <p className="text-gray-500 max-w-lg mb-8">
            {isLockedByReviewer 
              ? `A Reviewer has accepted your submission. Evaluation is in progress.` 
          ...
          {isLockedByReviewer ? (
            <div className="mt-8 flex flex-col items-center gap-2">
              <div className="flex items-center text-blue-700 text-sm bg-blue-50 px-5 py-2.5 rounded-full border border-blue-100 font-medium">
                ...
                Accepted by reviewer. Changes locked.
              </div>
            </div>
          ) : (
            <Button variant="outline" className="mt-8 border-red-200 text-red-600 hover:bg-red-50" onClick={handleRecall}>Recall Submission (Edit)</Button>
          )}
        </div>
      ) : (
        /* Assessment View */
            ...
            <Card className="flex-1 flex flex-col shadow-md border-0 h-full">
              {relevantQuestions.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                  ...
                  <p className="text-gray-500">No questions configured for this level.</p>
                </div>
              ) : (
                <div className="p-8 flex-1 flex flex-col">
                  <AnimatePresence mode="wait">
                    <motion.div key={currentQuestion.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.3 }} className="flex-1 flex flex-col">
                      <div className="mb-6">
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex gap-2">
                            {/* Level Badge in Question Header (Main View) */}
                          ...
                          <button onClick={() => setShowInfo(!showInfo)} className="p-2 rounded-full text-gray-400 hover:text-blue-600 hover:bg-blue-50"><Info className="w-5 h-5" /></button>
                        </div>
                            ...
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                              <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 mb-6">
                                ...
                                <ul className="list-disc list-inside space-y-1">
                                  {currentQuestion.expectations?.map((point: string, i: number) => (
                                    <li key={i} className="text-sm text-blue-700 leading-relaxed">{point}</li>
                      ...
                      <div className="space-y-8 flex-1">
                        ...
                        <Textarea label="Founder Notes / Context" rows={3} value={founderNotes[currentQuestion.id] || ''} onChange={(e) => handleNoteChange(e.target.value)} />
                        <div className="relative">
                          <Input label="Evidence Link" placeholder="https://..." value={evidenceLinks[currentQuestion.id] || ''} onChange={(e) => handleLinkChange(e.target.value)} />
                          <Link className="absolute right-3 top-9 w-4 h-4 text-gray-400 pointer-events-none" />
                        </div>
                        <div>
                          <label className="text-sm font-medium text-gray-700 block mb-2">Evidence File</label>
                          <div onClick={handleFileClick} className={`border-2 border-dashed rounded-lg p-6 flex flex-col items-center justify-center text-center cursor-pointer transition-colors ${evidenceFiles[currentQuestion.id] ? 'border-green-300 bg-green-50 hover:bg-green-100' : 'border-gray-300 hover:bg-gray-50'}`}>
                            {evidenceFiles[currentQuestion.id] ? (
                              <div className="flex items-center gap-2"><FileText className="w-5 h-5 text-green-600" /><span className="text-sm font-medium text-gray-900">{evidenceFiles[currentQuestion.id]}</span><button onClick={handleRemoveFile} className="z-10 p-1 bg-white rounded-full text-red-500 hover:bg-red-50"><X className="w-3 h-3" /></button></div>
                            ) : (
                              <><div className="w-10 h-10 bg-blue-100 rounded-full flex items-center justify-center mb-3"><Upload className="w-5 h-5 text-blue-600" /></div><p className="text-sm font-medium text-gray-900">Click to upload</p></>
                            )}
                          </div>
                        </div>
          ...
          <div className="w-full lg:w-80 flex flex-col">
            <Card className="h-full flex flex-col border-0 shadow-md">
                    ...
                    <button key={q.id} onClick={() => setCurrentQuestionIndex(idx)} className={`w-full text-left p-4 hover:bg-gray-50 transition-colors flex items-start space-x-3 ${idx === currentQuestionIndex ? 'bg-blue-50 border-l-4 border-blue-600' : 'border-l-4 border-transparent'}`}>
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-medium text-gray-900">{q.category}</p>
                          {/* FIX: ADDED LEVEL BADGE HERE */}
                          {isBaseline && <span className="text-[10px] bg-slate-100 px-1.5 py-0.5 rounded-full text-slate-500 font-bold border border-slate-200">L{q.airlLevel}</span>}
                        </div>
              ...
              <div className="p-6 border-t border-gray-100 bg-gray-50 rounded-b-lg space-y-3">
                ...
                <Button className="w-full" disabled={relevantQuestions.length === 0 || completedQuestions.length < totalQuestions} leftIcon={<Lock className="w-4 h-4" />} onClick={handleFinalSubmit}>Submit for AIRL {targetLevel}</Button>
              </div>
            </Card>
          </div>
        </div>
