...
import {
  ...
  Beaker,
  GraduationCap,
  MessageSquare // Added for proposal notification icon
} from "lucide-react";
...
export function Header({ title, user, userRole }: HeaderProps) {
  ...
  const [notifications, setNotifications] = useState([
    {
      ...
      message: "AgriDrone wants to collaborate with GreenField Tech.",
      time: "2h ago",
      read: false,
      type: 'collab',
  ...
  useEffect(() => {
    ...
    const foundProjects = projects.filter(p => p.name.toLowerCase().includes(lowerQuery) || p.domain.toLowerCase().includes(lowerQuery)).map(p => ({ ...p, type: 'Project', icon: Rocket, link: `/founder/project/${p.id}` }));
    const foundStartups = startups.filter(s => s.name.toLowerCase().includes(lowerQuery)).map(s => ({ ...s, type: 'Startup', icon: Building2, link: '/admin/portfolio' }));
    const foundFacilities = facilities.filter(f => f.name.toLowerCase().includes(lowerQuery)).map(f => ({ ...f, type: 'Facility', icon: Beaker, link: '/founder/facilities' }));
    const foundMentors = mentors.filter(m => m.name.toLowerCase().includes(lowerQuery) || m.domain.toLowerCase().includes(lowerQuery)).map(m => ({ ...m, type: 'Mentor', icon: GraduationCap, link: '/founder/mentors' }));
          ...
          {showResults && searchResults.length > 0 && (
            <div className="absolute top-full left-0 mt-2 w-full bg-white rounded-lg shadow-xl border border-gray-100 overflow-hidden py-2 z-50">
              ...
              {searchResults.map((result, idx) => (
                <button key={`${result.type}-${idx}`} onClick={() => handleResultClick(result.link)} className="w-full text-left px-4 py-3 hover:bg-gray-50 flex items-center gap-3 transition-colors">
                  <div className={`p-2 rounded-full bg-gray-100 text-gray-600`}><result.icon className="w-4 h-4" /></div>
                  <div><p className="text-sm font-medium text-gray-900">{result.name}</p><p className="text-xs text-gray-500">{result.type} • {result.domain || result.type}</p></div>
                </button>
              ))}
            </div>
          )}
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-xl border border-gray-100 py-1 z-50">
              <div className="px-4 py-2 border-b border-gray-50 flex justify-between items-center">
                ...
                <button className="text-xs text-blue-600 hover:underline" onClick={() => setNotifications(n => n.map(i => ({...i, read: true})))}>Mark all read</button>
              </div>
                ...
                {notifications.map(notif => (
                  ...
                  >
                    <div className="flex gap-3">
                      <div className={`mt-1 p-1.5 rounded-full ${notif.type === 'collab' ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-blue-600'}`}>
                        {notif.type === 'collab' ? <MessageSquare className="w-3 h-3"/> : <Bell className="w-3 h-3"/>}
                      </div>
                      <div>
        ...
        <div className="relative group">
          <button className="flex items-center space-x-3 border-l border-gray-200 pl-6 outline-none">
            <div className="w-8 h-8 rounded-full overflow-hidden ring-2 ring-gray-100 bg-slate-100 flex items-center justify-center">
              {currentUser.avatar ? <img src={currentUser.avatar} alt={currentUser.name} className="w-full h-full object-cover" /> : <span className="text-sm font-bold text-slate-500">{currentUser.name.charAt(0)}</span>}
            </div>
          ...
          <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-gray-100 py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right z-50">
            ...
            <Link to={getProfilePath(userRole)} className="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-gray-50"><UserIcon className="w-4 h-4 mr-2" /> Profile</Link>
            <button onClick={handleLogout} className="flex w-full items-center px-4 py-2 text-sm text-red-600 hover:bg-red-50"><LogOut className="w-4 h-4 mr-2" /> Sign out</button>
          </div>
        </div>
      </div>
    </header>
